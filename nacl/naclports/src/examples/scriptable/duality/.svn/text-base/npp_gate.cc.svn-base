...
NPError NPP_New(NPMIMEType mime_type,
                ...
                char* argv[],
                NPSavedData* saved) {
  printf("Duality: NPP_New was called!\n");
  fflush(stdout);
  if (instance == NULL)
...
NPError NPP_Destroy(NPP instance, NPSavedData** save) {
  printf("Duality: NPP_Destroy was called!\n");
  fflush(stdout);
  if (NULL == instance)
    return NPERR_INVALID_INSTANCE_ERROR;
...
NPObject *NPP_GetScriptableInstance(NPP instance) {
  printf("Duality: NPP_GetScriptableInstance was called!\n");
  fflush(stdout);
...
NPError NPP_GetValue(NPP instance, NPPVariable variable, void* ret_value) {
  printf("Duality: NPP_GetValue was called!\n");
  fflush(stdout);
...
int16_t NPP_HandleEvent(NPP instance, void* event) {
  printf("Duality: NPP_HandleEvent was called!\n");
  fflush(stdout);
  return 0;
}
...
NPError NPP_SetWindow(NPP instance, NPWindow* window) {
  printf("Duality: NPP_SetWindow was called!\n");
  fflush(stdout);
  return NPERR_NO_ERROR;
}
...
NPError InitializePluginFunctions(NPPluginFuncs* plugin_funcs) {
  printf("Duality: InitializePluginFunctions was called!\n");
  fflush(stdout);
